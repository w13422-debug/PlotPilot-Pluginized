...
def _safe_child(root: Path, child: Path) -> Path:
    ...
    for part in child.relative_to(root).parts:
        ...
        if current.is_symlink():
            raise PackageStoreError("symlink package-store path is forbidden", path=str(current))
    return child
...
class PackageStore:
    ...
    def __init__(self, root: str | os.PathLike[str], *, limits: PackageLimits | None = None) -> None:
    ...
    def _load_registry(self) -> dict[tuple[str, str], dict[str, str]]:
        ...
        try:
            ...
            raise PackageStoreError(f"invalid package registry: {exc}", path=str(self.registry_path)) from exc
        if not isinstance(raw, Mapping) or raw.get("schema") != _REGISTRY_SCHEMA or not isinstance(raw.get("releases"), list):
            raise PackageStoreError("invalid package registry shape", path=str(self.registry_path))
        ...
        for entry in raw["releases"]:
            ...
            if old is not None and old != value:
                raise PackageConflictError("package registry contains conflicting identities")
            result[key] = value
        return result

    def _write_registry(self) -> None:
        entries = sorted(self._registry.values(), key=lambda item: (item["plugin_id"].encode("utf-8"), item["version"].encode("utf-8")))
        payload = {"schema": _REGISTRY_SCHEMA, "releases": entries}
        tmp = self.registry_path.with_name(f".{self.registry_path.name}.{uuid.uuid4().hex}.tmp")
        try:
    ...
    def package_path(self, plugin_id: str, version: str, package_hash: str) -> Path:
        ...
        return _safe_child(
            self.root,
            self.packages_root / _component(plugin_id, label="plugin_id") / _component(version, label="version") / package_hash,
        )
    ...
    def _exclusive(self) -> Iterator[None]:
        with self._lock:
            # Refresh on each mutation so another PackageStore object in this
            # process cannot make this instance use a stale identity index.
            self._registry = self._load_registry()
            yield
    ...
    def _materialize(self, package: VerifiedPackage, destination: Path) -> None:
        ...
        for directory, dirs, _ in os.walk(destination, topdown=False):
            for name in dirs:
                try:
                    Path(directory, name).chmod(stat.S_IRUSR | stat.S_IXUSR | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
                except OSError:
                    pass
        try:
    ...
    def publish(
        ...
        *,
        ...
        if expected_package_hash is not None and verified.package_hash != expected_package_hash:
            raise PackageIntegrityError("package_hash does not match expected identity")
        if expected_release_id is not None and verified.release_id != expected_release_id:
        ...
        with self._exclusive():
            ...
            if old is not None and old["package_hash"] != verified.package_hash:
                raise PackageConflictError(
                    ...
                    details={"existing": old["package_hash"], "incoming": verified.package_hash},
                )
            destination = self.package_path(*verified.identity)
            if destination.exists() or destination.is_symlink():
                if destination.is_symlink() or not destination.is_dir():
                    raise PackageStoreError("published package path is not a directory", path=str(destination))
                try:
                    existing = verify_package(destination, limits=self.limits)
                except ContractError as exc:
                    raise PackageIntegrityError(f"published package failed integrity verification: {exc}") from exc
                if existing.identity != verified.identity or existing.release_id != verified.release_id:
                    raise PackageConflictError("published package identity does not match destination")
                if old is None:
            ...
            try:
                ...
                if not destination.is_dir():
                    raise PackageStoreError("package publication raced with a non-directory")
                existing = verify_package(destination, limits=self.limits)
                if existing.identity != verified.identity:
                    raise PackageConflictError("package publication raced with a different identity")
                verified = existing
            self._registry[key] = entry
            try:
                self._write_registry()
    ...
    def _find_entry(self, plugin_id: str, version: str, package_hash: str | None) -> dict[str, str]:
        self._registry = self._load_registry()
        key = (plugin_id, version)
        entry = self._registry.get(key)
        if entry is not None:
            if package_hash is not None and package_hash != entry["package_hash"]:
                raise PackageConflictError("requested package hash differs from the registered release")
            return entry
        ...
        parent = self.packages_root / _component(plugin_id, label="plugin_id") / _component(version, label="version")
        _safe_child(self.root, parent)
        if not parent.is_dir() or parent.is_symlink():
            raise FileNotFoundError((plugin_id, version))
        candidates = [child for child in parent.iterdir() if child.is_dir() and not child.is_symlink()]
        ...
        if candidate.plugin_id != plugin_id or candidate.version != version or (package_hash and candidate.package_hash != package_hash):
            raise PackageIntegrityError("package path does not match its manifest identity")
        return self._entry_for(candidate)

    def get(self, plugin_id: str, version: str, package_hash: str | None = None) -> VerifiedPackage:
        with self._lock:
    ...
    def has(self, plugin_id: str, version: str, package_hash: str | None = None) -> bool:
        try:
    ...
    def read_file(self, plugin_id: str, version: str, relative_path: str, package_hash: str | None = None) -> bytes:
        package = self.get(plugin_id, version, package_hash)
        return package.read_bytes(relative_path)

    open_file = read_file

    def list_releases(self, *, verify: bool = True) -> tuple[VerifiedPackage | Mapping[str, str], ...]:
        with self._lock:
            self._registry = self._load_registry()
            entries = sorted(self._registry.values(), key=lambda item: (item["plugin_id"].encode("utf-8"), item["version"].encode("utf-8")))
            if not verify:
                return tuple(MappingProxyType(dict(entry)) for entry in entries)
            return tuple(self.get(entry["plugin_id"], entry["version"], entry["package_hash"]) for entry in entries)

    releases = list_releases

    def package_url(self, plugin_id: str, version: str, relative_path: str, package_hash: str | None = None) -> str:
        package = self.get(plugin_id, version, package_hash)
