...
def _fact(
    workspace="ws", kind="character", entity="c1", revision="rev-1", digest="a" * 64
):
    return FactRef(workspace, kind, entity, revision, digest)


def test_frozen_manifest_schema_accepts_the_installable_planning_packages():
    schema = json.loads(
        (ROOT / "contracts/json-schema/plugin-manifest-v1.schema.json").read_text(
            "utf-8"
        )
    )
    validator = jsonschema.Draft202012Validator(schema)
    fixture = json.loads(
        (ROOT / "contracts/examples/fixtures/plugin-manifest-code.json").read_text(
            "utf-8"
        )
    )
    validator.validate(fixture)
    for name in ("project-planner", "story-state"):
        manifest = json.loads(
            (ROOT / "first-party-plugins" / name / "plugin.json").read_text("utf-8")
        )
...
def test_mixed_known_unknown_sections_fail_closed_before_any_draft():
    with pytest.raises(ValueError, match="outline_enhancement"):
        build_candidate_drafts(
            _plan(), _binding(), {"bible": {}, "outline_enhancement": {}}, run_id="bad"
        )
...
def test_payload_sources_and_inputs_are_deep_copied_and_frozen():
    ...
    draft = build_candidate_drafts(
        _plan(), _binding(), {"bible": content}, run_id="run", source_refs=sources
    )[0]
...
def test_ordered_lineage_sources_and_result_mode_are_in_draft_identity():
    kwargs = {
        "planning_input": _plan(),
        "binding": _binding(),
        "generated": {"world": {}},
        "run_id": "synth",
    }
    first = build_candidate_drafts(
        **kwargs,
        parent_candidate_ids=("a", "b"),
        source_refs=({"id": "s1"},),
        result_mode="synthesize",
    )[0]
    reversed_parents = build_candidate_drafts(
        **kwargs,
        parent_candidate_ids=("b", "a"),
        source_refs=({"id": "s1"},),
        result_mode="synthesize",
    )[0]
    changed_source = build_candidate_drafts(
        **kwargs,
        parent_candidate_ids=("a", "b"),
        source_refs=({"id": "s2"},),
        result_mode="synthesize",
    )[0]
    separate = build_candidate_drafts(
        **kwargs, source_refs=({"id": "s1"},), result_mode="separate"
    )[0]
    assert (
        len(
            {
                first.draft_key,
                reversed_parents.draft_key,
                changed_source.draft_key,
                separate.draft_key,
            }
        )
        == 4
...
def test_projection_identity_includes_workspace_and_kind_and_edges_use_full_keys():
    facts = (
        _fact("ws-a", "character", "same", "rev-1", "a" * 64),
        _fact("ws-b", "character", "same", "rev-2", "b" * 64),
        _fact("ws-a", "item", "same", "rev-3", "c" * 64),
    )
...
def test_partition_and_retry_preserve_identity_and_retry_only_failures():
    ...
    assert failed_items_for_retry((ok, failed), retry_id="try-2") == (
        ("op-1", "item-fail", "try-2"),
    )
