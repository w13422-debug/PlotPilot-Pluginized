...
export function presentJobState(state: JobState): StatusPresentation {
  switch (state) {
    ...
    case 'partial': return { label: '部分完成', tone: 'warning', description: '已有部分结果，可检查后重试' }
    case 'failed': return { label: '失败', tone: 'error', description: '任务失败，可重新尝试' }
...
export function presentStepState(state: string): StatusPresentation {
  switch (state) {
    ...
    case 'interrupted': return { label: '已中断', tone: 'warning', description: '步骤被中断，可查看任务详情' }
...
export function presentAttemptState(state: string): StatusPresentation {
  switch (state) {
    ...
    case 'cancelling': return { label: '取消中', tone: 'warning', description: 'Attempt 正在安全停止' }
    case 'succeeded':
    case 'completed': return { label: '已完成', tone: 'success', description: 'Attempt 已完成' }
    case 'partial': return { label: '部分完成', tone: 'warning', description: 'Attempt 产生了部分结果' }
    ...
    case 'interrupted': return { label: '已中断', tone: 'warning', description: 'Attempt 被中断' }
    case 'suspended': return { label: '已挂起', tone: 'warning', description: 'Attempt 已挂起，等待恢复条件' }
    case 'fenced': return { label: '已隔离', tone: 'error', description: 'Attempt 已被 fencing 隔离' }
...
export function jobProgress(snapshot: Pick<JobDrawerItem, 'steps'>): { completed: number; total: number } {
  const completedStates = new Set(['succeeded', 'completed', 'partial', 'failed', 'cancelled', 'interrupted', 'skipped'])
  return {
