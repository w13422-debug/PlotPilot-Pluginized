...
import unicodeCasefoldContract from '../../../contracts/unicode-casefold-v1.json' with { type: 'json' }
...
const CASEFOLD_TABLE = unicodeCasefoldContract.mappings as Record<string, string>
const NFC_DECOMPOSITION = unicodeCasefoldContract.nfc_decomposition as Record<string, number[]>
const NFC_COMBINING_CLASS = unicodeCasefoldContract.nfc_combining_class as Record<string, number>
const NFC_COMPOSITION = unicodeCasefoldContract.nfc_composition as Record<string, number>
const HANGUL_PROFILE = {
  s_base: 0xAC00,
  l_base: 0x1100,
  v_base: 0x1161,
  t_base: 0x11A7,
...
export function unicodeNfcCasefold(value: string): string {
  ...
  const normalized = unicodeNfc(value)
  let folded = ''
  for (const character of normalized) {
    const codepoint = (character.codePointAt(0) as number).toString(16).padStart(4, '0')
    folded += CASEFOLD_TABLE[codepoint] ?? character
  }
  return folded
}
...
export function unicodeNfc(value: string): string {
  ...
  const appendDecomposed = (codepoint: number): void => {
    const parts = NFC_DECOMPOSITION[codepoint.toString(16).padStart(4, '0')]
    if (parts == null) {
      decomposed.push(codepoint)
      return
    }
  ...
  const combiningClass = (codepoint: number): number => NFC_COMBINING_CLASS[codepoint.toString(16).padStart(4, '0')] ?? 0
  ...
  for (const codepoint of decomposed) {
    ...
    while (index > 0 && combiningClass(ordered[index - 1]) > ccc) index -= 1
  ...
  for (const codepoint of ordered) {
    ...
    if (starterIndex >= 0 && (lastClass === 0 || lastClass < ccc)) {
      composite = NFC_COMPOSITION[
        `${(composed[starterIndex] as number).toString(16).padStart(4, '0')}+${codepoint.toString(16).padStart(4, '0')}`
      ]
    }
    if (composite != null) {
      composed[starterIndex] = composite
      // The mark was absorbed into the starter.  Do not let its class block
      // the next canonical composition in the same combining segment.
    } else {
